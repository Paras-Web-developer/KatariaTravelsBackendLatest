...
class InvoiceMainRepository extends AppRepository
{
	...
	public function filter()
	{
		...
		if ($request->has('pending_payment_status_for_pax')) {
			$pendingStatus = (int) $request->pending_payment_status_for_pax;
		...
		if ($request->has('pending_payment_status_for_supplier')) {
			$pendingStatus = (int) $request->pending_payment_status_for_supplier;

			$model->where(function ($query) use ($pendingStatus) {
				$query->whereRaw(
					'
            (
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(airticket, "$.airticket_to_supplier.total")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(airticket, "$.airticket_to_supplier.amountPaid")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(airticket, "$.airticket_to_supplier.refund")), 0) +

                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(hotel, "$.hotel_to_supplier.total")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(hotel, "$.hotel_to_supplier.amountPaid")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(hotel, "$.hotel_to_supplier.refund")), 0) +

                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(land_package, "$.landpackage_to_supplier.total")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(land_package, "$.landpackage_to_supplier.amountPaid")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(land_package, "$.landpackage_to_supplier.refund")), 0) +

                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(cruise, "$.cruise_to_supplier.total")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(cruise, "$.cruise_to_supplier.amountPaid")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(cruise, "$.cruise_to_supplier.refund")), 0) +

                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(insurance, "$.insurance_to_supplier.total")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(insurance, "$.insurance_to_supplier.amountPaid")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(insurance, "$.insurance_to_supplier.refund")), 0) +

                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(misc, "$.misc_to_supplier.total")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(misc, "$.misc_to_supplier.amountPaid")), 0) -
                COALESCE(JSON_UNQUOTE(JSON_EXTRACT(misc, "$.misc_to_supplier.refund")), 0)
            ) ' . ($pendingStatus === 0 ? '= 0' : ($pendingStatus === 1 ? '> 0' : '< 0'))
				);
			});
		}

		if ($request->has('transation_type_id') && isset($request->transation_type_id)) {
			$transationTypeId = $request->transation_type_id;

			$model->where(function ($query) use ($transationTypeId) {
				$query->orWhere('airticket->airticket_from_pax->transation_type_id', $transationTypeId)
					->orWhere('airticket->airticket_to_supplier->transation_type_id', $transationTypeId)

					->orWhere('hotel->hotel_from_pax->transation_type_id', $transationTypeId)
					->orWhere('hotel->hotel_to_supplier->transation_type_id', $transationTypeId)

					->orWhere('cruise->cruise_from_pax->transation_type_id', $transationTypeId)
					->orWhere('cruise->cruise_to_supplier->transation_type_id', $transationTypeId)

					->orWhere('insurance->insurance_from_pax->transation_type_id', $transationTypeId)
					->orWhere('insurance->insurance_to_supplier->transation_type_id', $transationTypeId)

					->orWhere('land_package->landpackage_from_pax->transation_type_id', $transationTypeId)
					->orWhere('land_package->landpackage_to_supplier->transation_type_id', $transationTypeId)

					->orWhere('misc->misc_from_pax->transation_type_id', $transationTypeId)
